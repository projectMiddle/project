...
export const API_SERVER_HOST = "http://localhost:8080/approval";
...
export const postApproval = async (empNo, formData) => {
  const res = await axios.post(`${API_SERVER_HOST}/create/${empNo}`, formData, {
...
export const removeAppFile = async (fileName) => {
  const res = await axios.post(`${API_SERVER_HOST}/removeFile`, null, {
...
export const appLineEmployees = async () => {
  const res = await axios.get(`${API_SERVER_HOST}/employees`);
  return res.data;
};
...
export const postApprovalLine = async (appDocNo, approvers, references) => {
  ...
  const response = await axios.post(`${API_SERVER_HOST}/line`, payload); // 주소 직접 작성
  return response.data;
};

// 결재서류 상세정보 요청
export const fetchApprovalDetail = async (appDocNo) => {
  const res = await axios.get(`${API_SERVER_HOST}/detail/${appDocNo}`);
  return res.data;
};
...
export const processApproval = async ({ appDocNo, empNo, status, comment }) => {
  ...
  const res = await axios.post(`${API_SERVER_HOST}/process/${appDocNo}`, payload);
  return res.data;
};

// 삭제 요청
export const deleteApproval = async (appDocNo) => {
  const response = await axios.delete(`${API_SERVER_HOST}/delete/${appDocNo}`);
  return response.data;
};

// 배지용 카운트 요청
export const fetchApprovalCategoryCounts = async (empNo) => {
  const res = await axios.get(`${API_SERVER_HOST}/categorycount`, {
    params: { empNo },
  });
  return res.data;
};