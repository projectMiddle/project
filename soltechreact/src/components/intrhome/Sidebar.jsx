...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    startOfWeek.setDate(
      today.getDate() - (today.getDay() === 0 ? 6 : dayOfWeek - 1)
    );
    ...
    <aside className="w-100 text-white bg-gray-200 flex flex-col">
      <div className="px-6 py-4 h-2/5 bg-gradient-to-b from-[#6b46c1] to-purple-400 mt-20 border-b-1 border-gray-300">
        <div className="px-6 py-4 flex flex-col items-center">
          ...
          <button className="mt-3 w-full rounded bg-white/1 py-1 text-sm">
            정보 변경
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 px-6 space-y-2 pt-5 bg-[#f5f5f5] h-15">
        <div className="flex gap-5 p-3">
          <button className="w-full py-2 rounded-4xl text-[18px] border-1 border-gray-300 font-bold bg-blue-500 cursor-pointer hover:bg-blue-600">
            출근
          </button>
          <button className="w-full py-2 rounded-4xl text-[18px] border-1 border-gray-300 font-bold bg-red-600 cursor-pointer hover:bg-red-700">퇴근</button>
        </div>
...
function SideLink({ Icon, label, to }) {
  return (
    <Link
      to={to}
      className="flex items-center gap-2 px-2 py-1 text-black text-[16px] hover:bg-gray-300/20 rounded"
    >
      <Icon size={18} /> {label}
    </Link>
  );
