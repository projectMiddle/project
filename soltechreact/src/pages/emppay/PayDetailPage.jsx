...
export default function PayDetailPage() {
  ...
  const format = (num) =>
    num != null ? Number(num).toLocaleString("ko-KR") + " 원" : "";

  const [year, month] = pay.payMonth ? pay.payMonth.split("-") : [];

  return (
    <div>
      <div className="bg-[#6b46c1] text-white font-bold text-[17px] pl-5 py-[14px]">
        급여명세서 - 상세
      </div>
      <div className="p-6">
        <div className=" mx-auto bg-white shadow-2xl p-8 rounded-xl border border-gray-300">
          ...
          <div className="text-right font-bold text-lg text-gray-800 mb-6">
            실지급액:{" "}
            <span className="text-blue-700">{format(pay.payNetSalary)}</span>
          </div>
          {/* 근무시간 요약 */}
          ...
          <section className="text-sm border border-gray-300">
            <h4 className="text-base font-semibold text-gray-800 mb-3 p-4 bg-gray-100 pl-8">
              계산방법 (산출식)
            </h4>
            ...
            <div className="bg-white p-5 shadow-sm">
              <ul className="space-y-2 text-gray-700">
                ...
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    직급별 차등 (MGR: 20%, AM: 10%, JM: 5%) → 중간값 10% 적용
                  </span>
                </li>
                <li className="flex items-start gap-2">
