...
export function filterEmployees(list, keyword, jobFilter, deptFilter) {
  return list.filter((emp) => {
    ...
    const matchesName = keyword ? name.toLowerCase().includes(keyword.toLowerCase()) : true;

    const matchesJob = jobFilter ? job === jobFilter : true;
...
// 카테고리 목록 (공통 상수로 분리)
export const approvalCategories = ["기안서", "보고서", "연차신청서", "출장신청서", "영수증"];
